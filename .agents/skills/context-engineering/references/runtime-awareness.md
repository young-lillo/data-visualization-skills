...
Monitor usage limits and context window utilization in real-time to optimize Codex sessions.
...
|----------|--------|----------|
| macOS | Keychain | runtime credentials entry |
| Windows | File | runtime credentials file |
| Linux | File | runtime credentials file |
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "node scripts/hooks/usage-context-awareness.cjs"
        }]
      }
    ]
  }
...
|-------|-------|----------|
| No usage limits shown | No OAuth token | Refresh runtime authentication |
