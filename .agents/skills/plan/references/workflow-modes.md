...
**Invocation:** Run `plan red-team {plan-directory-path}`.
```
plan red-team {plan-directory-path}
```
...
| `auto` | Run `plan validate {plan-directory-path}` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Run:
```
plan validate {plan-directory-path}
```
...
After creating, mark the plan as active in the current runtime context and pass the plan directory path to every subagent.
