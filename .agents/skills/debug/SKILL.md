...
- Replace legacy task-tool flows with plan files under `plans/` plus `update_plan` when progress tracking is needed.
- Replace slash-command examples with plain skill names or the active `$dv-*` workflow.
...
Track investigation pipelines via plan files plus `update_plan`. The legacy task-based pattern remains useful conceptually, but Codex execution should default to markdown plans and concise runtime progress tracking.
...
Visual verification of frontend implementations via Chrome MCP or `ck:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Scouting:** `ck:scout` for finding relevant files
